...
| LLM    | **Claude 4 Opus @ Anthropic**                                                                                                                                | 原生 Tool Calling；注册即用 ↗︎\[turn0search6]. |
| 工作流    | **LangGraph**                                                                                                                                                | DAG 编排多 Agent ↗︎\[turn0search10]。       |
| RAG 检索 | **Azure AI Search Free** (3 索引) ↗︎\[turn0search7]                                                                                                            | 职位文档向量化。                                |
| 职位来源   | Apify StepStone Scraper ↗︎\[turn0search0]；SerpAPI Google Jobs ↗︎\[turn0search1]；JobsPikr Sandbox ↗︎\[turn0search2]；CoreSignal Self‑Service ↗︎\[turn0search3] | 全部自助领取 key，无信用卡。                        |
| 简历解析   | **APILayer Resume Parser Free** (100 req/月) ↗︎\[turn0search4]                                                                                                | PDF/Word → JSON。                        |
| PDF 渲染 | **PDFMonkey Free** (20 PDF/月) ↗︎\[turn0search5]                                                                                                              | Markdown/HTML → PDF URL。                |
...
| `ParseResumeTool`    | 用户上传简历      | `parse_resume`                            | 调 APILayer `/parse` → 返回结构化 JSON         |
...
```python
payload={"document":{"template_id":TEMPLATE_ID,"payload":{...}}}
url = "https://api.pdfmonkey.io/api/v1/documents"
headers={"Authorization":f"Bearer {PDFMONKEY_KEY}"}
pdf_url = httpx.post(url,json=payload,headers=headers).json()["data"]["attributes"]["download_url"]
```
...
* **Chat History**：LangChain `ConversationBufferMemory` 持久化到 SQLite。"清除聊天记录" = DELETE WHERE user\_id=X。
...
| 4 | Indeed 替代实时职位                    | **JobsPikr Sandbox**        | 2 k req/月     | [https://www.jobspikr.com/](https://www.jobspikr.com/)                                                               | `JOBSPIKR_KEY`                              |
| 5 | LinkedIn 全量职位                    | **CoreSignal Self‑Service** | 100 rows demo | [https://coresignal.com/blog/api-self-service-tutorial/](https://coresignal.com/blog/api-self-service-tutorial/)     | `CORESIGNAL_KEY`                            |
| 6 | 简历解析                             | **APILayer Resume Parser**  | 100 req/月     | [https://apilayer.com/marketplace/resume\_parser-api](https://apilayer.com/marketplace/resume_parser-api)            | `APILAYER_RESUME_KEY`                       |
| 7 | PDF 生成                           | **PDFMonkey**               | 20 PDF/月      | [https://pdfmonkey.io/pricing/](https://pdfmonkey.io/pricing/)                                                       | `PDFMONKEY_KEY`                             |
| 8 | 搜索索引                             | **Azure AI Search Free**    | 3 索引 / 50 MB  | [https://learn.microsoft.com/azure/search/search-sku-tier](https://learn.microsoft.com/azure/search/search-sku-tier) | `AZURE_SEARCH_ENDPOINT`, `AZURE_SEARCH_KEY` |
| 9 | 登录 & OIDC                        | **Google OAuth Testing**    | 100 用户        | [https://console.cloud.google.com/apis/credentials](https://console.cloud.google.com/apis/credentials)               | `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`  |
...
```python
@tool(name="write_pdf", args_schema=PDFSchema)
async def write_pdf(markdown:str)->str:
    data = {"document": {"template": "md", "payload": {"content": markdown}}}
    r = await client.post("https://api.pdfmonkey.io/api/v1/documents", json=data)
...
* **为什么需要**：职位列表实时变化，防止用户收藏或检索索引中出现 404 / 已下线岗位。
